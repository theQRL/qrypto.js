...
describe('dist bundle smoke tests', () => {
  describe('ESM (dist/mjs/mldsa87.js)', () => {
    it('imports and generates a keypair', async () => {
      const { stdout } = await run(`
        import { cryptoSignKeypair, CryptoPublicKeyBytes, CryptoSecretKeyBytes } from './dist/mjs/mldsa87.js';
    ...
    it('sign and verify round-trip', async () => {
      const { stdout } = await run(`
        import { cryptoSignKeypair, cryptoSignSignature, cryptoSignVerify, CryptoBytes, CryptoPublicKeyBytes, CryptoSecretKeyBytes } from './dist/mjs/mldsa87.js';
        const pk = new Uint8Array(CryptoPublicKeyBytes);
        const sk = new Uint8Array(CryptoSecretKeyBytes);
        cryptoSignKeypair(null, pk, sk);
  ...
  describe('CJS (dist/cjs/mldsa87.js)', () => {
    it('requires and generates a keypair', async () => {
      const { stdout } = await run(`
        const { cryptoSignKeypair, CryptoPublicKeyBytes, CryptoSecretKeyBytes } = require('./dist/cjs/mldsa87.js');
        const pk = new Uint8Array(CryptoPublicKeyBytes);
        const sk = new Uint8Array(CryptoSecretKeyBytes);
        cryptoSignKeypair(null, pk, sk);
        console.log(pk.length === CryptoPublicKeyBytes && sk.length === CryptoSecretKeyBytes);
    ...
    it('sign and verify round-trip', async () => {
      const { stdout } = await run(`
        const { cryptoSignKeypair, cryptoSignSignature, cryptoSignVerify, CryptoBytes, CryptoPublicKeyBytes, CryptoSecretKeyBytes } = require('./dist/cjs/mldsa87.js');
        const pk = new Uint8Array(CryptoPublicKeyBytes);
        const sk = new Uint8Array(CryptoSecretKeyBytes);
        cryptoSignKeypair(null, pk, sk);
        const msg = new TextEncoder().encode('test');
