...
import { Q, N, D, ETA, TAU, GAMMA1, GAMMA2, BETA, OMEGA, CTILDEBytes, CRHBytes, SeedBytes } from '../src/const.js';
...
describe('D5-8: polyChallenge() determinism and structure', function () {
  it('same seed produces identical polynomial', function () {
    const seed = new Uint8Array(CTILDEBytes);
    seed.fill(0xab);
    const c1 = new Poly();
    const c2 = new Poly();
  ...
  it('different seeds produce different polynomials', function () {
    const s1 = new Uint8Array(CTILDEBytes);
    s1.fill(0x01);
    const s2 = new Uint8Array(CTILDEBytes);
    s2.fill(0x02);
    const c1 = new Poly();
    const c2 = new Poly();
    polyChallenge(c1, s1);
  ...
  it('output has exactly TAU nonzero coefficients', function () {
    for (let trial = 0; trial < 5; trial++) {
      const seed = new Uint8Array(CTILDEBytes);
      for (let i = 0; i < CTILDEBytes; i++) seed[i] = (trial * 31 + i) & 0xff;
      const c = new Poly();
  ...
  it('all nonzero coefficients are in {-1, 1}', function () {
    for (let trial = 0; trial < 5; trial++) {
      const seed = new Uint8Array(CTILDEBytes);
      for (let i = 0; i < CTILDEBytes; i++) seed[i] = (trial * 71 + i) & 0xff;
      const c = new Poly();
      polyChallenge(c, seed);
      for (let i = 0; i < N; i++) {
