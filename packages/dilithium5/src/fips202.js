...
function load64(x, xOffset) {
    ...
    for(let i=0;i<8;i++)
        r = BigInt.asUintN(64, r | BigInt.asUintN(64, BigInt(x[xOffset + i]) << BigInt(8*i)));
...
function store64(x, xOffset, u) {
    for(let i=0;i<8;i++)
        x[xOffset + i] = Number((u >> BigInt(8*i)) & 0xffn);
}
...
function keccakAbsorb(s, pos, r, input) {
    ...
    while(pos+inLen >= r) {
        for(i=pos;i<r;i++)
            s[Math.floor(i/8)] = BigInt.asUintN(64, s[Math.floor(i/8)] ^ (BigInt(input[inputOffset++]) << BigInt(8*(i%8))));
        inLen -= r-pos;
...
function keccakSqueeze(out, s, pos, r) {
    ...
    while(outLen) {
        ...
        for(i=pos;i < r && i < pos+outLen; i++)
            out[outputOffset++] = s[Math.floor(i/8)] >> BigInt(8*(i%8));
        outLen -= i-pos;
        pos = i;
    }
...
function keccakAbsorbOnce(s, r, input, p) {
    ...
    while(inLen >= r) {
        for(i=0;i<Math.floor(r/8);i++)
            s[i] = BigInt.asUintN(64, s[i] ^ load64(input, inputOffset+8*i));
        inputOffset += r;
        inLen -= r;
        KeccakF1600StatePermute(s);
    }
    ...
    for(i=0;i<inLen;i++)
        s[Math.floor(i/8)] = BigInt.asUintN(64, s[Math.floor(i/8)] ^ (BigInt(input[inputOffset+i]) << BigInt(8*(i%8))));
...
function keccakSqueezeBlocks(output, outputOffset, nBlocks, s, r) {
    while(nBlocks) {
        ...
        for(let i=0;i<Math.floor(r/8);i++)
            store64(output, outputOffset + 8*i, s[i]);
        outputOffset += r;
        nBlocks -= 1;
    }
}
