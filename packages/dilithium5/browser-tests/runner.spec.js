...
test('dilithium5 browser test suite', async ({ page }) => {
  const logs = [];
  page.on('console', (msg) => {
    const line = `[console:${msg.type()}] ${msg.text()}`;
    logs.push(line);
    console.log(`[dilithium5] ${line}`);
  });
  ...
  await page.goto('/packages/dilithium5/browser-tests/runner.html');
  ...
  while (true) {
    ...
    if (elapsed - lastLog >= logIntervalMs) {
      ...
      if (progress) {
        console.log(
          `[dilithium5] ${progress.passed} passed, ${progress.failed} failed, ` +
            `${progress.pending} pending, ${progress.started} started. ` +
            `Current: ${progress.current || 'unknown'}`,
        );
      } else {
        console.log('[dilithium5] waiting for mocha to start...');
      }
      lastLog = elapsed;
    }
